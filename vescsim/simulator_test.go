...
func TestSimulatorEndToEnd(t *testing.T) {
	...
	for range 10 {
		...
		if rt.State.RunState == StateRunning {
			break
		}
	}
	if rt.State.RunState != StateRunning {
	...
	if rt.State.StopCondition != StopSwitchFull {
...
func TestCANBusPing(t *testing.T) {
	...
	resp := sim.HandleCommand([]byte{byte(CommPingCAN)})
	if resp == nil {
	...
	if resp[0] != byte(CommPingCAN) {
		t.Errorf("PingCAN cmd = 0x%02x, want 0x%02x", resp[0], byte(CommPingCAN))
	}
	if resp[1] != 0 {
...
func TestCANForwardToExpress(t *testing.T) {
	...
	payload := []byte{byte(CommForwardCAN), 253, byte(CommFWVersion)}
	resp := sim.HandleCommand(payload)
	if resp == nil {
		t.Fatal("ForwardCAN to Express returned nil")
	}
	if resp[0] != byte(CommFWVersion) {
	...
	hwType := HWType(resp[idx])
	if hwType != HWTypeVESCExpress {
		t.Errorf("HW type = %d, want %d (VESCExpress)", hwType, HWTypeVESCExpress)
	}
}
...
func TestCANForwardToSelf(t *testing.T) {
	...
	payload := []byte{byte(CommForwardCAN), 0, byte(CommFWVersion)}
	resp := sim.HandleCommand(payload)
	if resp == nil {
	...
	hwType := HWType(resp[idx])
	if hwType != HWTypeVESC {
		t.Errorf("HW type = %d, want %d (VESC)", hwType, HWTypeVESC)
	}
}
...
func TestCANForwardUnknownDevice(t *testing.T) {
	...
	payload := []byte{byte(CommForwardCAN), 99, byte(CommFWVersion)}
	resp := sim.HandleCommand(payload)
	if resp != nil {
...
func TestRefloatInstallFlow(t *testing.T) {
	...
	resp := sim.HandleCommand([]byte{byte(CommCustomAppData), PackageMagic, byte(CommandInfo)})
	if resp != nil {
	...
	eraseResp := sim.HandleCommand([]byte{byte(CommEraseNewApp)})
	if eraseResp == nil || eraseResp[0] != byte(CommEraseNewApp) {
		t.Fatal("COMM_ERASE_NEW_APP should return ack")
	}

	writeResp := sim.HandleCommand([]byte{byte(CommWriteNewAppData), 0x00}) // dummy data
	if writeResp == nil || writeResp[0] != byte(CommWriteNewAppData) {
